...
export default function SubmitTask() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setTimeout(async () => {
      ...
      const ai_feedback = {
        score: 7.5,
        strengths: "Clean logic and readable code.",
        improvements: "Add more error handling, and test edge cases.",
        verdict: "Good job, but can be improved!"
      };
    ...
    <div className="flex flex-col min-h-screen bg-gray-50">
      <main className="flex-1 p-4 md:p-8 flex items-center justify-center">
        <Card className="w-full max-w-2xl">
          <CardHeader>
            <CardTitle className="text-3xl font-bold">New Task Evaluation</CardTitle>
            <CardDescription>
          ...
          <form onSubmit={handleSubmit}>
            <CardContent className="grid gap-6">
              ...
              {error && (
                <div className="text-red-600 bg-red-100 p-3 rounded-md border border-red-200">{error}</div>
              )}
            </CardContent>
            <CardFooter>
